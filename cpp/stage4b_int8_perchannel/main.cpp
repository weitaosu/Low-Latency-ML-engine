...
static Stage4bConfig cfg_for(const std::string& size) {
    Stage4bConfig c;
    if (size == "medium") {
...
int main(int argc, char** argv) {
    ...
    std::string bin   = repo + "/models/" + size + "_int8_4b.bin";
    std::string in_p  = repo + "/data/inputs_1k.npy";
    std::string out_p = repo + "/data/outputs_1k_" + size + ".npy";
    std::string csv   = repo + "/results/csvs/stage4b_" + size + "_int8_perchannel.csv";

    Stage4bConfig cfg = cfg_for(size);
    Stage4bEngine engine(bin, cfg);
    ...
    MeasureConfig mc;
    mc.stage = "stage4b"; mc.size_ = size; mc.precision = "int8_perchannel";
