...
static Stage0Config cfg_for(const std::string& size) {
    Stage0Config c;
    if (size == "medium") {
...
int main(int argc, char** argv) {
    ...
    std::string repo = env ? env : ".";   // set REPO=/path/to/repo or run from repo root
    ...
    std::string csv   = repo + "/results/csvs/stage0_" + size + "_fp32.csv";

    Stage0Config cfg = cfg_for(size);
    Stage0Engine engine(bin, cfg);

    // 1. Correctness
    auto res = check_correctness(in_p, out_p, cfg.seq_len, cfg.n_vars,
        [&](const float* x, float* y){ engine.forward(x, y); }, 1e-3);
    if (!res.passed) { fprintf(stderr, "correctness FAILED — refuse to benchmark\n"); return 1; }

    // 2. Benchmark — iter counts overridable for fast notebook runs
    ...
    MeasureConfig mc;
    mc.stage = "stage0"; mc.size_ = size; mc.precision = "fp32";
