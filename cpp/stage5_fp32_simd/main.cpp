...
static Stage5FConfig cfg_for(const std::string& size) {
    Stage5FConfig c;
    if (size == "medium") {
...
int main(int argc, char** argv) {
    ...
    std::string csv   = repo + "/results/csvs/stage5_fp32_" + size + ".csv";

    Stage5FConfig cfg = cfg_for(size);
    Stage5FEngine engine(bin, cfg);
    ...
    MeasureConfig mc;
    mc.stage = "stage5_fp32"; mc.size_ = size; mc.precision = "fp32";
