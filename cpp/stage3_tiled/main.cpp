...
static Stage3Config cfg_for(const std::string& size) {
    Stage3Config c;
    if (size == "medium") {
...
int main(int argc, char** argv) {
    ...
    std::string csv   = repo + "/results/csvs/stage3_" + size + "_fp32.csv";

    Stage3Config cfg = cfg_for(size);
    Stage3Engine engine(bin, cfg);
    ...
    MeasureConfig mc;
    mc.stage = "stage3"; mc.size_ = size; mc.precision = "fp32";
