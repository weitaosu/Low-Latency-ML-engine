...
\paragraph{Scope.}
All measurements are on a single CPU (Intel Alder Lake P-core with AVX-VNNI), at batch size 1, without KV cache, on an encoder-only architecture at a single sequence length of 96.
Latency conclusions transfer cleanly to any deployment with the same tensor shapes and a similar cache hierarchy, but absolute numbers will differ on AMD Zen, ARM, or pre-Alder-Lake Intel hardware.

\paragraph{Single dataset.}
All accuracy claims are evaluated on ETTh1 only.
The latency claims are dataset-independent (they are functions of model shape, not data distribution), but quantization MSE drift is not, and we do not characterize it on other distributions.

\paragraph{Stage 5 INT8 SIMD not implemented.}
This was the natural next stage and is the most plausible path to closing the gap to ONNX Runtime on the medium model.
The non-VNNI implementation path requires \texttt{\_mm256\_maddubs\_epi16} chained with \texttt{\_mm256\_madd\_epi16} per $K$-block, while the VNNI path collapses to a single \texttt{\_mm256\_dpbusd\_avx\_epi32} per four-$K$-element step.
Correctness verification is more delicate than scalar INT8 because lane-mixing during the horizontal-sum reduction creates many opportunities for off-by-one mistakes that pass scalar-equivalent smoke tests but fail on real workloads.
We did not attempt the implementation here.

\paragraph{Rust port not delivered.}
A Rust port was scoped early as a comparison point.
The original motivation came from cross-language energy-efficiency studies that consistently rank C, C++, and Rust as the most efficient languages~\citep{pereira2021ranking}; the question of whether Rust's safety model adds measurable single-call latency overhead remains an open side-experiment.

\paragraph{Model under-fits ETTh1.}
Test MSE 0.874 (small) and 0.905 (medium) versus the published best of 0.376 for FEDformer.
This is the published vanilla-Transformer ceiling on ETTh1 (Informer at 0.941~\citep{nie2023patchtst}); the latency findings are independent of forecasting accuracy.

\paragraph{Statistical caveats.}
Bootstrap confidence intervals assume sample independence, but adjacent iterations in our 100{,}000-sample runs are not strictly independent: there is short-range serial correlation from L2 and TLB warm-up effects.
The CIs are therefore a conservative-leaning estimate of the truth, and their tightness should not be over-interpreted to claim significance for sub-microsecond stage-to-stage deltas.
